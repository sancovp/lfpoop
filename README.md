...
| `lfpoop/classify.py` | **the classification tier (MVP ②)**: every free name lands in one alphabet — chain / ring:<r> / config / ambient:pure / ambient:effect / demand — and the alphabet decides the action: the `curry_plan` issues curry_to_slot, bind_ring, leave, isolate_effect, or demand per name (the one law at name grain); verdicts (pure/effectful/config_dependent/ring_coupled/open) exact; effects LOCALIZED to the single block that carries them |
...
python3 tests/test_classify.py       # the classification tier: alphabets drive the curry
```
